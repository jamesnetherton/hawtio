...
public class HawtioConfiguration {
    ...
    public FilterRegistrationBean loginRedirectFilter(Redirector redirector) {
        final String[] unsecuredPaths = {"/auth","/css","/img","/js","/hawtconfig.json","/jolokia","/keycloak","/libs","/oauth","/user"};
        final FilterRegistrationBean filter = new FilterRegistrationBean();
    ...
    private String prependContextPath(String... paths) {
        List<String> contextPaths = Arrays.stream(paths).map(path -> hawtioPath + path).collect(Collectors.toList());
//        List<String> contextPaths = Arrays.stream(paths).collect(Collectors.toList());
